    ...
    <div class="body_div">
        <div>
            <el-breadcrumb separator-class="el-icon-arrow-right">
                ...
                <el-breadcrumb-item>人才称号申报</el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        ...
        <div class="table_div">
            <el-table border :data="getTableData" stripe header-cell-class-name="my-header-cell-class"
                ...
                <el-table-column prop="category" label="申报称号"></el-table-column>
                <el-table-column prop="unit" label="申报单位"></el-table-column>
                <el-table-column prop="date" label="申报时间"></el-table-column>
...
import DialogForm from "@/view/dialog/dialogten.vue";
...
if(store.department=='人事处管理员'){
    store.routerPath = '/index1/eleven1'
}else{
store.routerPath='/index/eleven'
}
...
const searchdata = () => {
    ...
    if (value.value == "") {
        console.log("--------")
        axios.get("/selectTalentTitleApplicationByName", {
            params: {
                name: input.value,
        ...
        axios.get("/selectTalentTitleApplicationByNameAndState", {
            params: {
                state: value.value,
                name: input.value,
                department: store.department
...
const edit = (id: number) => {
    console.log(id);
    axios.get("/selectTalentTitleApplicationById", {
        params: {
            id: id
        }
    }).then((response) => {
...
const tongguo = (id: number) => {
    console.log(id);
    axios.get("/updateTalentTitleApplicationById", {
        params: {
            id: id,
            state:'已通过'
        }
...
const select = () => {
    ...
    if (store.department == "人事处管理员") {
        axios
            .get("/selectAllTalentTitleApplication")
            .then((response: AxiosResponse<any>) => {
                tableData.value = response.data;
                triggerRef(tableData);
                loading.value = !loading.value;
        ...
        axios
            .get("/selectTalentTitleApplication", {
                params: {
                    department: store.department
                }
            })
...
const dataFilter = () => {
    ...
    if (input.value == "") {
        ...
        axios.get("/selectTalentTitleApplicationByState", {
            params: {
                state: value.value,
                department: store.department
            }
