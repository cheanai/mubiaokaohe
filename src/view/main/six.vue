    ...
    <div class="body_div">
        <div>
            <el-breadcrumb separator-class="el-icon-arrow-right">
                ...
                <el-breadcrumb-item>公需课培训</el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        ...
        <div class="table_div">
            <el-table border :data="getTableData" stripe header-cell-class-name="my-header-cell-class"
                ...
                <el-table-column prop="trainingName" label="公需课名称"></el-table-column>
                <el-table-column prop="trainingTime" label="培训时间"><template #default="scope">
                        <div style="display: flex; align-items: center" v-if=scope.row.time>
                            <el-icon>
                                <timer />
                            </el-icon>
                            <span style="margin-left: 10px">{{ scope.row.trainingTime }}</span>
                        </div>
                    </template></el-table-column>
                <el-table-column prop="trainingHours" label="培训时长"></el-table-column>
...
import DialogForm from "@/view/dialog/dialogfive.vue";
import { useMain } from "@/store/home";
import axios from "@/api/axiosInstance";
import { AxiosResponse, AxiosError } from "axios";
const store = useMain();
if(store.department=='人事处管理员'){
    store.routerPath = '/index1/six1'
}else{
store.routerPath='/index/six'}
...
const searchdata = () => {
    ...
    if (value.value == "") {
        console.log("--------")
        axios.get("/selectPublicCourseTrainingByTeacherName", {
            params: {
        ...
        axios.get("/selectPublicCourseTrainingByNameAndState", {
            params: {
                state: value.value,
                teacherName: input.value,
                department: store.department
...
const edit = (id: number) => {
    console.log(id);
    axios.get("/selectPublicCourseTrainingById", {
        params: {
            id: id
        }
    }).then((response) => {
...
const tongguo = (id: number) => {
    console.log(id);
    axios.get("/updatePublicCourseTrainingById", {
        params: {
            id: id,
            state:'已通过'
        }
...
const select = () => {
    ...
    if (store.department == "人事处管理员") {
        axios
            .get("/selectAllPublicCourseTraining")
            .then((response: AxiosResponse<any>) => {
                tableData.value = response.data;
                triggerRef(tableData);
                loading.value = !loading.value;
        ...
        axios
            .get("/selectPublicCourseTraining", {
                params: {
                    department: store.department
                }
            })
...
const dataFilter = () => {
    if (input.value == "") {
        ...
        axios.get("/selectPublicCourseTrainingByState", {
            params: {
                state: value.value,
                department: store.department
            }
