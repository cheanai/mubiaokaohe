    ...
    <div class="body_div">
        <div>
            <el-breadcrumb separator-class="el-icon-arrow-right">
                ...
                <el-breadcrumb-item>新增厅局级称号</el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        ...
        <div class="table_div">
            <el-table border :data="getTableData" stripe header-cell-class-name="my-header-cell-class"
                ...
                <el-table-column prop="talentTitle" label="称号"></el-table-column>
...
import DialogForm from "@/view/dialog/dialognine.vue";
import { useMain } from "@/store/home";
import axios from "@/api/axiosInstance";
import { AxiosResponse, AxiosError } from "axios";
const store = useMain();
if(store.department=='人事处管理员'){
    store.routerPath = '/index1/ten1'
}else{
store.routerPath='/index/ten'}
...
const searchdata = () => {
    ...
    if (value.value == "") {
        console.log("--------")
        axios.get("/selectTalentTeacherByName", {
            params: {
                name: input.value,
        ...
        axios.get("/selectTalentTeacherByNameAndState", {
            params: {
                state: value.value,
                name: input.value,
                department: store.department
...
const edit = (id: number) => {
    console.log(id);
    axios.get("/selectTalentTeacherById", {
        params: {
            id: id
        }
    }).then((response) => {
...
const tongguo = (id: number) => {
    console.log(id);
    axios.get("/updateTalentTeacherById", {
        params: {
            id: id,
            state:'已通过'
        }
...
const dataFilter = () => {
    ...
    if (input.value == "") {
        ...
        loading.value = !loading.value;
        axios.get("/selectTalentTeacherByState", {
            params: {
                state: value.value,
                department: store.department
            }
